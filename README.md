...
- **工具注册与执行治理**：统一管理工具 Schema、JSON 严格解码、参数校验、执行超时、结果摘要和结构化落库；自建评测集（100 条 / 4 类场景 / 含 20% 对抗用例）**工具调用成功率 95.3%**，剩余失败均被参数校验/超时/上游错误结构化捕获
- **Evidence Guard 证据完整性校验**：在生成最终结论前校验必需工具证据是否齐全，缺失时自动注入重试指令；**必需证据完整率从 66.0% 提升至 86.0%（+20pp, n=100）**
- **Skill 化分析模板**：按运营场景配置工具白名单、必需证据、输出章节和风险提示，运行时限制 LLM 可见工具集合；**越权工具调用率从 34.3% 降至 0%，输出章节命中率从 20.0% 提升至 85.0%**
